...
namespace WordleClient.libraries.ingame
{
    public class GameInstance
    {
        ...
        {
            this.targetRecord = new WDBRecord
            {
                TOKEN = testWord,
        ...
        {
            if (guess.Length != targetRecord.TOKEN.Length) throw new ArgumentException("Guess length does not match target word length.");
            ...
            for (int i = 0; i < guess.Length; i++)
            {
                if (result.Get(i) == TriState.MATCH) continue;
                bool found = false;
