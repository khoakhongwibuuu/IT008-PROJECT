...
namespace WordleClient.libraries.ingame
{
    public class HintGetter
    {
        ...
        private static readonly HashSet<char> ConsonantSet = [.. Consonants];
        private static char? GetRandomVowel(string input)
        {
            if (string.IsNullOrEmpty(input)) return null;
            var vowels = input.Where(c => char.IsLetter(c) && VowelSet.Contains(char.ToUpperInvariant(c))).ToList();
        ...
        {
            if (string.IsNullOrEmpty(input)) return null;
            var consonants = input.Where(c => char.IsLetter(c) && ConsonantSet.Contains(char.ToUpperInvariant(c))).ToList();
        ...
        public static Hint GetHint(string token, int seed)
        {
            ...
            return c == null ? new Hint(HintType.Absent, null, even ? AbsentType.VOWEL : AbsentType.CONSONANT) : new Hint(HintType.Present, c, null);
        }
    }
}